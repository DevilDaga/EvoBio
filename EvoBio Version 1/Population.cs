...
namespace EvoBio_Version_1
{
	public class Population
	{
		...
		private static List<T> CreateStartingDistribution<T>
			(
			double repMean, double repSd,
			double lethalMean, double lethalSd,
			int count
			)
				...
				foreach ( var ind in all )
				{
					...
					switch ( ind )
					{
						case WildIndividual _:
							...
							{
								lethal = Math.Max ( 0, Utility.NextGaussian ( ind.originalLethal, V.K ) ),
								rep = Math.Max ( 0, Utility.NextGaussian ( ind.originalRep, V.J ) )
							};
						...
						case MutantIndividual _:
							...
							{
								lethal = Math.Max ( 0, Utility.NextGaussian ( ind.originalLethal, V.M ) ),
								rep = Math.Max ( 0, Utility.NextGaussian ( ind.originalRep, V.L ) )
							};
							chosen.Add ( offspring );
							break;
						...
						case AmpIndividual _:
							...
							{
								lethal = Math.Max ( 0, Utility.NextGaussian ( ind.originalLethal, V.O ) ),
								rep = Math.Max ( 0, Utility.NextGaussian ( ind.originalRep, V.N ) )
							};
							chosen.Add ( offspring );
							break;
					}
